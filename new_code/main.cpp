...
# include "headers/edge_thinning.h"
...
public:
    ...
    Planar_Graph (String _input_folder, String _name_base, String _ext, String _output_folder)
    {
    ...
    bool Image_to_Graph (int image_ind)
    {
        ...
        values_pixels = Threshold( image_gray, size_small );

        // Generating the mask
        mask = cv::Mat_<bool>( image_sizes.y, image_sizes.x, false );
        for ( auto v : values_pixels ) mask.at<bool>( v.second ) = true;
        if (save_images) Save_Mask( image, mask, name + "_mask" + std::to_string( size_small ) + ".png" );
        ...
        Edge_Thinning(image, mask, min_areas, save_images, name);
...
int main ()
{
    String input_folder = "./input/";   // Path to input folder
    String output_folder = "./output/"; // Path to output folder
    ...
    for ( int image_ind = start_ind; image_ind < start_ind + num_graphs; image_ind++ )
        if ( ! planar_graph.Image_to_Graph( image_ind ) ) continue;
    return 1;
}
