...
// Remove an external pixel
bool Remove_External_Pixel_Iterative (Point point, Mat_<bool>& mask, bool& check)
{
    // exceptional cases
    if ( point.x < 0 or point.y < 0 or point.x >= mask.cols or point.y >= mask.rows ) return false;
    if ( Remove_Isolated_Pixel( point, mask ) ) return true;
    ...
    for ( int i = 0; i < shifts8.size(); i++ )
        if ( neighbors[i] != neighbors[ (i+1) % neighbors.size() ] ) changes++;
    //if ( debug ) std::cout<<"\n"<<point<<" n="<<num_neighbors<<" c="<<changes;
    if ( num_neighbors > 3 and num_neighbors < 6 and changes == 2)
    {
...
// Function removes all external pixels
bool Remove_External_Pixels_Iterative (Mat_<bool>& mask)
{
    bool check;
    do{
        check = false;
        for ( int row = 0; row < mask.rows; row++ )
            for ( int col = 0; col < mask.cols; col++ )
                if ( mask.at<bool>( Point( col, row ) ) )
                    Remove_External_Pixel_Iterative( Point( col, row ), mask, check);
    }while(check);
...
// Function removes all additional junction pixels
bool Remove_Extra_Junctions (Mat_<bool>& mask)
{
    for ( int row = 0; row < mask.rows; row++ )
        for ( int col = 0; col < mask.cols; col++ )
            if ( mask.at<bool>( Point( col, row ) ) )
            {
                ...
                for ( int i = 0; i < shifts8.size(); i++ )
                    if ( neighbors[i] != neighbors[ (i+1) % neighbors.size() ] ) changes++;
                if (num_neighbors > 2 and changes == 2)
                    mask.at<bool>( point ) = false;
            }
    return true;
}
...
bool Edge_Thinning (Mat const& image, Mat_<bool>& mask, std::vector<int>& min_areas, bool& save_images, std::string name)
{
    ...
    Remove_External_Pixels_Iterative( mask );
    //if ( save_images ) Save_Mask( image, mask, name + "_intermediate_skeleton.png" );


    Remove_Extra_Junctions(mask);
    if ( save_images ) Save_Mask( image, mask, name + "_skeleton.png" );
    ...
    if ( save_images ){
        ...
        cv::imwrite(  name + "_skeleton_original.png" , image_mask );
        cv::imwrite(  name + "_skeleton_no_holes.png" , im );
    }
